        ...
        <h2>
          ...
          @auth
            ...
            <form class="ml-2 d-inline" action="/create-follow/{{$sharedData['username']}}" method="POST">
              @csrf
              <button class="btn btn-outline-primary btn-sm">Follow <i class="fas fa-user-plus"></i></button>           
            </form>
            @endif
            ...
            @csrf
            <button class="btn btn-outline-danger btn-sm">Stop Following <i class="fas fa-user-plus"></i></button>           
           </form>
            @endif

            @if(Auth::user()->username == $sharedData['username'])
            <a href="/manage-avatar" class="btn btn-outline-secondary btn-sm">Manage Avatar</a>
            @endif
        @endauth
        </h2>
