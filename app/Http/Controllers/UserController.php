...
{
    public function storeAvatar(Request $request): \Illuminate\Http\RedirectResponse
    {
        $request->validate([
        ...
        Storage::put('public/avatarfolder/' . $filename, $imgData);
    ...
    private function getSharedData($user)
    {
        ...
        if (Auth::check()) {
            $currentlyFollowing = Follow::where('user_id', Auth::user()->id)->where('followeduser', $user->id)->count();
        }

        View::share('sharedData', ['currentlyFollowing' => $currentlyFollowing, 'avatar' => $user->avatar, 'username' => $user->username, 'postCount' => $user->posts()->count(), 'followerCount' => $user->followers()->count(), 'followingCount' => $user->followingUsers()->count()]);
    }
